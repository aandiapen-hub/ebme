...
def test_delivery_update_view_post_unsuccessful(client, user, delivery, part):
    ...
    data = {
        ...
        # One form in the formset
        'tbldeliverylines_set-0-item': str(part().pk),  # Assuming you have at least one part in the database
...
@pytest.mark.django_db
def test_invoice_create_view_post_successful(client, user,purchase_order, po_line, invoice_status):
    user = user()
    permission = Permission.objects.get(codename="add_tblinvoices")
    user.user_permissions.add(permission)
    client.force_login(user)
