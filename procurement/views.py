...
#import Models
from .models import (TblInvoices, TblPurchaseOrder,TblPoLines, PoView,
                     PoView, Outstandngdeliveriesview,TblDeliveries,
                     TblDeliveryLines
)
...
from .forms import (PoCreateForm,PoLineFormset,
                    DeliveryLineFormset,
                    InvoiceCreateForm
)
...
class PoTableView(LoginRequiredMixin, PermissionRequiredMixin,
                  FilteredTableView):
...
class PoCreateView(LoginRequiredMixin, PermissionRequiredMixin,
                   CreateView):
    model = TblPurchaseOrder
...
class PoUpdateView(LoginRequiredMixin, PermissionRequiredMixin,
                   UpdateView):
    model = TblPurchaseOrder
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            context['formset'] = PoLineFormset(instance=self.object,supplier_id=supplier_id)
        return context
    
    def post(self, request, *args, **kwargs):
        self.object = self.get_object()
        form = self.get_form()
        formset = PoLineFormset(self.request.POST,instance=self.object)  # You'll need to define this
...
class PoDetailView(LoginRequiredMixin, PermissionRequiredMixin,
                   DetailView):
    model = TblPurchaseOrder
...
class PoDeleteView(LoginRequiredMixin, PermissionRequiredMixin,
                   DeleteView):
    model = TblPurchaseOrder
...
#PO lines views
class PoLinesListView(LoginRequiredMixin, PermissionRequiredMixin,
                      ListView):
    model = TblPoLines
...
#PO lines views
class GeneratePurchaseOrder(LoginRequiredMixin, PermissionRequiredMixin,
                            DetailView):
    model = TblPurchaseOrder
    context_object_name = 'po_lines'
    permission_required = 'procurement.view_tblpurchaseorder'
...
#Deliveries
class DeliveriesListView(LoginRequiredMixin, PermissionRequiredMixin,
                         ListView):
    model = TblDeliveries
...
class DeliveryLinesListView(LoginRequiredMixin, PermissionRequiredMixin,
                            ListView):
...
#Deliveries
class OutstandingItemsListView(LoginRequiredMixin, PermissionRequiredMixin,
                               ListView):
    ...
    permission_required = ['procurement.view_tbldeliveries','procurement.view_tblpurchaseorder']
...
class DeliveryCreateView(LoginRequiredMixin, PermissionRequiredMixin,
                         CreateView):
    model = TblDeliveries
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            for item in outstanding_items:
                ...
                try:
                    delivered_item['qty'] = int([d.get('qty') for d in delivered_items if d['item'] == item.part_number][0])
                except:
                    delivered_item['qty'] = item.outstanding
                initial.append(delivered_item)
                
            context['formset'] = DeliveryLineFormset(instance=self.object, initial=initial,extra=len(initial)+2)
    ...
    def form_valid(self, form, formset): 
            with transaction.atomic():
                ...
                if temp_file_group and temp_file_group != 'None':
                    files = TemporaryUpload.objects.filter(user=self.request.user,group=temp_file_group)
                    if files:
                        delivery_note_file = SaveTempFiles(
                            temp_files=files, 
                            link_row= self.object.pk,
                            link_table= self.model._meta.db_table,
                            document_type=TblDocumentTypes.objects.filter(document_type_name='Delivery Note').first(),
...
class DeliveryUpdateView(LoginRequiredMixin, PermissionRequiredMixin,
                         UpdateView):
    model = TblDeliveries
    ...
    def post(self, request, *args, **kwargs): 
            ...
            form = self.get_form() 
            po = self.object.po.po_id
            formset = DeliveryLineFormset(self.request.POST,instance=self.object, po=po)  # You'll need to define this 
            if form.is_valid() and formset.is_valid(): 
                return self.form_valid(form, formset) 
            else: 
                return self.form_invalid(form, formset) 
...
class DeliveryDeleteView(LoginRequiredMixin,
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            if self.request.htmx:
                return render(self.request, 'partials/messages.html',status=200)  # HTMX expects 200 OK even if empty

            # Fallback redirect if not HTMX
            return HttpResponseRedirect(self.get_success_url())

        except ProtectedError:
            messages.warning(self.request, "This item cannot be deleted because it is linked to other records.")
            context = self.get_context_data(object=self.object)
            return self.render_to_response(context)


class DeliveryNoteReader(LoginRequiredMixin,
                         PermissionRequiredMixin,
                        View):
    permission_required = 'procurement.change_tbldeliveries'
    ...
    def get_success_url(self, **kwargs):
        return reverse('procurement:delivery_note_reader_output', kwargs={'temp_file_group':self.group})
...
class DeliveryNoteReaderOutput(LoginRequiredMixin, PermissionRequiredMixin,
                     TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not output:
            ...
            if delivery_note:
                delnotes = TblDeliveries.objects.filter(delivery_note_number__icontains=delivery_note)
                if delnotes.exists():
...
#invoices
class FilteredInvoiceTableView(LoginRequiredMixin,PermissionRequiredMixin,
                             FilteredTableView):
    model = TblInvoices
    paginate_by = 25
...
class InvoicesCreateView(LoginRequiredMixin, PermissionRequiredMixin,
                   CreateView):
    model = TblInvoices
    ...
    def get_initial(self):
        ...
        invoice_reader_output = get_extraction_results(self.request.user,temp_file_group)
                ...
                if temp_file_group and temp_file_group != 'None':
                    ...
                    if files:
                        invoice_file = SaveTempFiles(
                            ...
                            link_row= self.object.pk,
                            link_table= self.model._meta.db_table,
                            document_type=TblDocumentTypes.objects.filter(document_type_name='Invoice').first(),
...
class InvoicesDetailView(LoginRequiredMixin, PermissionRequiredMixin,
                   DetailView):
    model = TblInvoices
...
class InvoicesUpdateView(LoginRequiredMixin, PermissionRequiredMixin,
                   UpdateView):
    model = TblInvoices
    template_name = 'procurement/invoices_create.html'
    form_class = InvoiceCreateForm
    ...
    def get_success_url(self):
        return reverse('procurement:invoices_detail', kwargs={'pk':self.object.invoice_id})

class InvoicesDeleteView(LoginRequiredMixin, PermissionRequiredMixin,
                   DeleteView):
    model = TblInvoices
...
class InvoicesListView(LoginRequiredMixin, PermissionRequiredMixin,
                         ListView):
    model = TblInvoices
...
class InvoiceReader(LoginRequiredMixin,
                    PermissionRequiredMixin,
                    View):
    permission_required = 'procurement.change_tblinvoices'
    template_name = 'procurement/partials/delivery_note_reader.html'
    ...
    def get_success_url(self, **kwargs):
        return reverse('procurement:invoices_reader_output', kwargs={'temp_file_group':self.group})
...
class InvoiceReaderOutput(LoginRequiredMixin, PermissionRequiredMixin,
                     TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not output:
            ...
            if invoice_no:
                invoice = TblInvoices.objects.filter(invoice_no__icontains=invoice_no).first()
                if invoice:
