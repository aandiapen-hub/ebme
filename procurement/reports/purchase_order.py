...
def listPolines(data):
    ...
    for i in range(len(data)):
        po_lines.append(
            list(map(lambda attr: str(getattr(data[i], attr)), po_lines_headers))
        )
    return po_lines


def tableHeaderText():
    po_lines = ["Qty", "Item ref", "Item Description", "Unit Price", "Line Price"]
...
conditions = f"""1. Enter this order in accordance with the prices, terms, delivery method and
specifications listed above.\n
2. Please notifiy us immediately if you are unable to ship as specified.\n
3. Send all correspondence to:\n
    Customer Serivce, {os.getenv("COMPANY_ADDRESS")}
"""
...
def gen_purchase_order(data):
    ...
    data = [
        [
            Paragraph(cell, numberStyle()) if i >= 3 else cell
            for i, cell in enumerate(row)
        ]
        for row in data
    ]
