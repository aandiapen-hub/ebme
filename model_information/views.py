...
#import forms
from .forms import (ModelCreateForm, ModelQuickCreateForm,
                    BrandBulkUpdateForm,ModelBulkUpdateForm
...
from utils.generic_views import get_dynamic_table_class

# Create your views here.

#brand views
class FilteredBrandTableView(LoginRequiredMixin,
    ...
    table_class = None
    model = Tblbrands
    template_columns = {'actions' : 'model_information/tables/brandlist_buttons.html'}
...
class BrandCreateView(LoginRequiredMixin, PermissionRequiredMixin,
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["title"] = 'Create New Brand' 
        context["view_type"] = 'create'

        return context
    
class BrandDeleteView(LoginRequiredMixin, PermissionRequiredMixin,
                      DeleteView):
    model = Tblbrands
...
#model views
class FilteredModelTableView(LoginRequiredMixin, PermissionRequiredMixin,
                             FilteredTableView):
...
class ModelUpdateView(LoginRequiredMixin, PermissionRequiredMixin,
                      UpdateView):
    model = Tblmodel
    fields = "__all__"
    ...
    def get_success_url(self):
        return reverse("model_information:model_view", kwargs={'pk':self.object.modelid})
...
class ModelCreateView(LoginRequiredMixin,
                           PermissionRequiredMixin,
                           CreateView):
    model = Tblmodel
...
class ExistingModelListView(LoginRequiredMixin,
                        PermissionRequiredMixin,
                        ListView):
    model = Tblmodel
    permission_required = 'assets.view_tblmodel'
    ...
    def get_queryset(self):
        ...
        if modelname:
            queryset = queryset.filter(modelname__icontains=modelname).exclude(gtin__isnull=False)[:10]
            return queryset
        

    
class ModelDeleteView(LoginRequiredMixin, PermissionRequiredMixin,
                      DeleteView):
    model = Tblmodel
    template_name = "model_information/partials/modal.html"
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Return an error message as plain text (not JSON)
            context = self.get_context_data()
            context['error_message'] = f"An error occurred while deleting the model. Error Details: {str(e)}"
            return self.render_to_response(context)


class ModelDetailView(LoginRequiredMixin, PermissionRequiredMixin,
                      DetailView):
    model = Tblmodel
...
#category views
class FilteredCategoryTableView(LoginRequiredMixin, PermissionRequiredMixin,
                                FilteredTableView):
    model = Tblcategories
    paginate_by = 25
...
class CategoryDeleteView(LoginRequiredMixin, PermissionRequiredMixin,
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Return an error message as plain text (not JSON)
            context = self.get_context_data()
            context['error_message'] = f"An error occurred while deleting the category. Error Details: {str(e)}"
            return self.render_to_response(context)
...
class ChecklistsTableView(LoginRequiredMixin, PermissionRequiredMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        modelid = self.request.GET.get("modelid")
        if modelid:
            context['model'] = Tblmodel.objects.get(modelid=self.request.GET.get("modelid") )
        return context

class CheckUpdateView(LoginRequiredMixin, PermissionRequiredMixin,
                      UpdateView):
    model = Tblcheckslists
    fields = "__all__"
    template_name = "model_information/partials/modal.html"
...
class CheckDeleteView(LoginRequiredMixin, PermissionRequiredMixin,
                      DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Return an error message as plain text (not JSON)
            context = self.get_context_data()
            context['error_message'] = f"An error occurred while deleting the test. Error Details: {str(e)}"
            return self.render_to_response(context)
        
class CheckCreateView(LoginRequiredMixin, PermissionRequiredMixin,
                      CreateView):
    model = Tblcheckslists
    fields = "__all__"
    template_name = "model_information/partials/modal.html"
