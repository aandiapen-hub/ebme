...
from documents.models import (DocumentsView,
                              TblDocTableRef,
                              TblDocumentLinks, TblDocumentTypes,
                              TblDocuments, TemporaryUpload
                              )
...
def test_document_create_view_renders(client,user_setup,mocker):
    user = user_setup
    mocker.patch('documents.mixins.DocumentPermissionMixin.has_permission', return_value=True)
...
def test_document_create_view_post_successfully(client,user_setup,mocker):
    ...
    #test htmx
    test_file2 = SimpleUploadedFile("test.txt", b"Test content2 ", content_type="text/plain")
...
def test_document_create_view_post_dubplicated_document(client,user_setup,mocker):
    ...
    test_file2 = SimpleUploadedFile("test.txt", b"Test content", content_type="text/plain")
...
def test_document_link_update_view_post_unsuccessful(client,user_setup,mocker):
    ...
    assert 'An error occurred while updating the document' in response.context['error_message']
...
def test_temporary_upload_create_view_post_new_group(client,user_setup,mocker):
    ...
    client.force_login(user)
    mocker.patch('django.contrib.auth.mixins.PermissionRequiredMixin.has_permission', return_value=True)
    test_file = SimpleUploadedFile("test.pdf", b"test image data",content_type='application/pdf')
...
@pytest.mark.django_db
def test_temporary_upload_create_view_post_specific_group_htmx(client,user_setup,mocker):
    user = user_setup
    client.force_login(user)
    mocker.patch('django.contrib.auth.mixins.PermissionRequiredMixin.has_permission', return_value=True)
...
@pytest.mark.django_db
def test_temporary_upload_create_view_post_rejects_dissimilar_file_format(client,user_setup,mocker):
    user = user_setup
    client.force_login(user)
    mocker.patch('django.contrib.auth.mixins.PermissionRequiredMixin.has_permission', return_value=True)
...
@pytest.mark.django_db
def test_temporary_upload_create_view_poKst_existing_group_htmx(client,user_setup,mocker):
    user = user_setup
    client.force_login(user)
    mocker.patch('django.contrib.auth.mixins.PermissionRequiredMixin.has_permission', return_value=True)
    ...
    test_file = SimpleUploadedFile("test.pdf", b"test image data",content_type='image/jpeg')
...
def test_temporary_upload_create_view_post_invalid(client,user_setup,mocker):
    ...
    test_file = SimpleUploadedFile("test.pdf", b"test image data",content_type='text/plain')
...
def test_document_update_view_renders(client,user_setup,mocker):
    ...
    last_document = TblDocuments.objects.filter(document_id=document_link.document_id).first()
...
def test_save_temp_file_permanently(user_setup,client):
    ...
    temp_files = SaveTempFiles(temp_document,300, TblDocTableRef.objects.first().table_name)
    temp_files.save_all()
...
@pytest.mark.django_db
def test_link_temporary_document_view_post_successfully_htmx(client, user_setup,mocker):
    user = user_setup
    client.force_login(user)
    mocker.patch('django.contrib.auth.mixins.PermissionRequiredMixin.has_permission', return_value=True)
...
@pytest.mark.django_db
def test_link_temporary_document_view_post_successfully_multiple_images(client, user_setup,mocker):
    user = user_setup
    client.force_login(user)
    mocker.patch('django.contrib.auth.mixins.PermissionRequiredMixin.has_permission', return_value=True)
